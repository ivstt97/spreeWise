...
var PrecacheHandler = class extends MessageHandler {
  ...
  constructor({ assetCache, dataCache: dataCache2, documentCache: documentCache2, plugins, state }) {
    super({ plugins, state: {} });
  ...
  async _handleMessage(event) {
    const { data } = event;
    let dataCache2, documentCache2, assetCache;
...
var dynamicCacheName = "site-dynamic-v4";
var dataCache = Storage.open(dynamicCacheName, {
  ttl: 60 * 60 * 24 * 7 * 1e3
  // 7 days
});
var documentCache = Storage.open(staticCacheName);
var assetsCache = Storage.open("assets-cache", {
  ttl: 60 * 60 * 24 * 30 * 1e3
  // 30 days
});
self.addEventListener("install", (event) => {
